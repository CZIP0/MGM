...
class LidarGridMapper:
    def __init__(self):
        ...
        # Feliratkozás az Odometry és LiDAR topikokra
        lidar_sub = message_filters.Subscriber("/scan", LaserScan)
        odom_sub = message_filters.Subscriber("/odom", Odometry)  # AMCL helyett Odometry
    ...
    def callback(self, lidar_data, odom_data):
        ...
        # Robot pozíciója az Odometry-ből
        robot_position = odom_data.pose.pose.position
        robot_orientation = odom_data.pose.pose.orientation
        
        robot_x = int(robot_position.x / GRID_RESOLUTION) + GRID_SIZE // 2
        robot_y = int(robot_position.y / GRID_RESOLUTION) + GRID_SIZE // 2
        ...
        for i, distance in enumerate(lidar_data.ranges):
            ...
            # Transformációs mátrix alkalmazása
            try:
        ...
        current_state.pose.position.x = odom_data.pose.pose.position.x
        current_state.pose.position.y = odom_data.pose.pose.position.y
        current_state.pose.orientation = odom_data.pose.pose.orientation
